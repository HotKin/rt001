...
public class StrUtils extends StrKit {
	public final static Logger LOGGER = LoggerFactory.getLogger(StrUtils.class);

    public static String urlDecode(String string) {
        try {
            return URLDecoder.decode(string, RT.me().getConstants().getEncoding());
    ...
    public static String urlEncode(String string) {
        try {
            return URLEncoder.encode(string, RT.me().getConstants().getEncoding());
        } catch (UnsupportedEncodingException e) {
    ...
    public static String urlRedirect(String redirect) {
        try {
            redirect = new String(redirect.getBytes(RT.me().getConstants().getEncoding()), "ISO8859_1");
        } catch (UnsupportedEncodingException e) {
        	LOGGER.error("urlRedirect is error", e);
        }
        return redirect;
    }
